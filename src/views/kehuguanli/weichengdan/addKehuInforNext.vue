<template>
    <div class="addcontractList ku_addBox" >
        <el-row class="addhetongBox">
            <p class="crm_title">
                <i class="crm_line"></i>
                <span  @click="jumpkehu">客户列表</span>><span  @click="jumphetong">合同列表</span>><span>添加合同</span>
            </p>
            <el-col :span="24" class="consoleTitle">
                <span class="corporateName">{{ advertiser }}</span>
            </el-col>
...
<script>
    ...
    import { contract_add_new,contract_num,Roler_users ,contract_addru_new,productline,customer_addru,addcontacts} from '@/api/kehu';
    ...
    export default {
        data() {
            return {
                ...
                customer_addru:function(data){
                    customer_addru({
                        data:data
                    }).then(response => {
                        if(response.code == '200'){
                            for(let i = 0;i<dataInfor[1].names.length;i++){
                                dataInfor[1].names[i].customer_id =response.data.id;
                                addcontacts( dataInfor[1].names[i]);
                            }
                        ...
                        this.formData[0].advertiser = response.data.id;
...
//
                ...
                tan(data) {
                    this.$confirm(data, '提示', {
                        ...
                        this.disabledPull = true;
                        this.customer_addru(dataInfor[0][0]);
                    }).catch(() => {
                        this.$message({
                            type: 'info',
                            message: '已取消提交'
        ...
        mounted(){
            ...
            dataInfor =  JSON.parse(localStorage.getItem('kebaoAccounts'));
            this.submitUser = this.user.name;
            this.formData[0].market = Number(this.user.id);

            this.advertiser =  dataInfor[0][0].advertiser;
            this.guanggaozhu = dataInfor[0][0].advertiser;
            this.formData[0].submituser = this.user.id;
            this.formData[0].appname =dataInfor[0][0].appname;
...
//返回上一级
            back(){
                ...
                this.$router.push({ name: 'AddCustomerInformation' , query: { id:id}});
            },
        },
        filters:{
            fileFun(val){
