            ...
            <div>
                <el-table
                            ...
                            <div :class="{ textBorder:scope.row.audit == 2 }">
                                <div class="fandianBox">
                                    <div class="fandianName">
                                        <span class="firstSpan">账户：{{ scope.row.xf_fandian }}</span>
                                        <span>媒体：{{  scope.row.mtfd  }}</span>
                                    </div>
                                    <div class="fandianName">
                    ...
                    <el-table-column
                        ...
                        <template slot-scope="scope">
                            <div :class="{ textBorder:scope.row.audit == 2 }">
                                {{scope.row.shifu_money | currency('')}}
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column
                        ...
                        <template slot-scope="scope">
                            <div :class="{ textBorder:scope.row.audit == 2 }">
                                    <span v-if="scope.row.xf_account">
                ...
                yihuikuanxufei_list:function(page,num,val){
                    ...
                    yihuikuanxufei_list({
                        ...
                        for(let i = 0;i<response.data.data.length;i++){
                            dataList[i]["money"] = Number(dataList[i].money);
                            dataList[i]["tk_shifu"] = Number(dataList[i].tk_shifu);
                            dataList[i]['lirun_money'] =   Number(dataList[i].money)- Number(dataList[i].shifu_money);
                        }
                        this.tcqlr_sum = response.data.tcqlr_sum;
                        this.huikuanAll =  response.data.huikuan;

                        this.kehuTableLength =  response.data.totalCount;
                        this.pageSize =  this.search_list.caiwu_xufei_page?this.search_list.caiwu_xufei_page:20;
                        this.pageIndex = this.search_list.caiwu_xufei_num ?this.search_list.caiwu_xufei_num :1;
                ...
                yihuikuanxufei_listExcel:function(num){
                    ...
                    yihuikuanxufei_list({
                        ...
                        this.tableData =  response.data.data.filter(function(item,i) {
                            ...
                            item["tk_shifu"] = Number(item.tk_shifu);
                            if(item.xiaoshou){
                                item['xiaoshouName'] = item.xiaoshou.name;
                            }

                            if(item.xf_account){
                                item['lirun_money'] =  Number(item.money)-  Number(item.shifu_money);
                            }else{
                                item['lirun_money'] = '0';
                            }
                        ...
                        const tHeader = ['账户',this.$t('titles.customer'), '简称', '产品线','回款金额',
...
//表格
        ...
        methods: {
            ...
            renewList(){
                ...
                this.dataInfor.search['ktype'] = 'renew';

                this.dataInfor.search['Search_str'] = this.search_list.caiwu_xufei_name;
                this.dataInfor.search['date_type'] = this.search_list.caiwu_xufei_date_type?this.search_list.caiwu_xufei_date_type:1;
                this.dataInfor.search['start_date'] = this.search_list.caiwu_xufei_start_date;
                this.dataInfor.search['end_date'] = this.search_list.caiwu_xufei_end_date;
                this.inputText = this.search_list.caiwu_xufei_name;
                this.dataInput = [this.search_list.caiwu_xufei_start_date,this.search_list.caiwu_xufei_end_date];
                this.date_type = this.search_list.caiwu_xufei_date_type?this.search_list.caiwu_xufei_date_type:1;
                this.pageSize =  this.search_list.caiwu_xufei_page?this.search_list.caiwu_xufei_page:20;
                this.pageIndex = this.search_list.caiwu_xufei_num ?this.search_list.caiwu_xufei_num :1;
                this.kehuTableLength = this.search_list.caiwu_xufei_length ?this.search_list.caiwu_xufei_length :1;
                this.yihuikuanxufei_list(this.page,this.num,this.dataInfor);
            },
            searchDataFun(){
                this.searchData({
                    caiwu_xufei_name: this.dataInfor.search.Search_str,
                    caiwu_xufei_date_type: this.dataInfor.search.date_type,
                    caiwu_xufei_start_date: this.dataInfor.search.start_date,
                    caiwu_xufei_end_date: this.dataInfor.search.end_date,
                    caiwu_xufei_page:this.pageSize,
                    caiwu_xufei_num:this.pageIndex,
                    caiwu_xufei_length:  this.kehuTableLength
                });
            },
            //搜索
            searchInput(){
...
//清除
            searchClear(){
                ...
                this.searchData({
                    caiwu_xufei_name:'',
                    caiwu_xufei_date_type: '',
                    caiwu_xufei_start_date: '',
                    caiwu_xufei_end_date: '',
                });
            },
        },
        created() {
