...
<script>
    ...
    import { customer_contract_list_new,productline,contract_add,hetongAddru} from '@/api/kehu';
                    ...
                    if(this.$route.query.type == 1){
                        customer_contract_list_new({
                            ...
                            require.ensure([], () => {
                                ...
                                const tHeader = [this.$t('titles.customer'), '产品线', '创建时间','归档','提交人'];
                                const filterVal = ['advertiseraccount', 'lineName','ctime', 'guidang','submitname'];
                        ...
                        xieyicustomer_contract_list({
                            ...
                            require.ensure([], () => {
                                const { export_json_to_excel } = require('@/vendor/Export2Excel');
                                const tHeader = [this.$t('titles.customer'), '产品线', '创建时间', '合同属性','提交人'];
                                const filterVal = ['advertiseraccount', 'prlin','ctime', 'contract_state','submitname'];
                                const list = this.tableData;
                                const data = this.formatJson(filterVal, list);
...
//                this.$router.push({ name: 'addTheContract_customer' , query: { id:val.advertiser,htId: val.contract_no}});
                this.$router.push({ name: 'newContract',query: { id:val.advertiser,htId: val.id} });
            },
            addhetong(){
                if(this.$route.query.type == 1){
                    this.$router.push({ name: 'MediaaddTheContract' , query: { id: this.routeId}});
                }

            },
            addhetong_kj(){
//                this.$router.push({ name: 'addTheContract_xy' , query: { id: this.routeId}});
                this.$router.push({ name: 'newContract',query: { id:this.routeId} });
            },
            changedate(){},
            searchproperty(){
