<template>
    <div class="fenpei_balanceList" >
        <el-row class="fenpei_balanceBox">
            <p class="crm_title">
                <i class="crm_line"></i>
                <span  @click="jumpCustomerList">媒体列表</span>><span @click="jumpConsole">客户控制台</span>><span>分配余额</span>
            </p>
                ...
                <div>
                    <el-table
                        ...
                        <el-table-column
                                ...
                                label="合同金额">
                            <template slot-scope="scope">
                                {{scope.row.contract_money  | currency('')}}
                            </template>
                        </el-table-column>
                        <el-table-column
                                prop="address"
                        ...
                        <el-table-column
                            ...
                            <template slot-scope="scope">
                                <div class="operation">
                                    <el-input v-model="scope.row.inputData" placeholder="请输入内容" type="number"></el-input>
...
<script>
    ...
    import { fpdakuanAll ,fpdakuanru} from '@/api/meiti';
...
//表格
                fenpei_balanceTable:[],
                fpdakuanAll:function(id){
                    fpdakuanAll({
                        id:id,
                    }).then(response => {
                        response.contract_list =      response.contract_list.filter(function(item,i) {
                             item['inputData'] = 0;
                            item['hetong_yu_e'] =Number(item.dakuan)+Number(item.bukuan)- Number(item.yu_e);
                            return item
                        });
                        this.titleName = response.costomer.advertiser;
                        this.fenpei_number = response.costomer.dakuan_undistributed_yu_e;
